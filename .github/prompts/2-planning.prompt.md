...
- The new `## UPDATE` should be appended to the end of the existing `# UPDATES` section (aka before `# AFFECTED MODULES`).
...
- Python modules depend on each other, by just implementing the task it may not be enough. Find out what will be affected.
...
- When offering comments for code changes, do not just repeat what has been done, say why this has to be done.
  ...
  - Do not say something like `i += 1  # add one to i`, which offers no extra information. Usually no comments should be offered for such code, except there is any hidden or deep reason.
...
- Your goal is to write a planning document to `Copilot_Planning.md`. DO NOT update any other file including source code.
...
## Step 3.1. Fill the "# AFFECTED MODULES" section

- Project structure can be found in `REPO-ROOT/README.md`.
- When creating `Copilot_Planning.md` from the first time, copy `# AFFECTED MODULES` section from `Copilot_Task.md`.
- When updating `Copilot_Planning.md`, carefully review `# AFFECTED MODULES` and fix this section if necessary.
  - According to what you need to change, selectively fill this section using bullet-list, each item could be one of the following:
    - `- Install/update dependencies in <requirements.txt or pyproject.toml>`.
    - `- Run tests for module <MODULE-NAME>`.
      - When specific test files are relevant, mention them.
    - `- Run linting and type checking for module <MODULE-NAME>`.
    - If a task could be skipped conditionally, write down what should trigger the running, usually it should be a list of files or modules. But you can describe it in a high level instead of listing all files.
    - If a task should always run, use `Always Run` instead of just `Run`.
