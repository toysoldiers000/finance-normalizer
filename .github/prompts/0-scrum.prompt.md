...
- Each task should be:
  ...
  - For a test planning task:
    - The test plan is about writing unit test cases using pytest.
  ...
  - Well ordered. At the end of each task, the project should be able to run tests.
...
- If there is no `# !!!VERIFIED!!!` in `Copilot_Execution.md`, it means you failed to deliver the task, either the code did not pass tests or some test cases failed. My edit will also reflect the final solution to the task.
